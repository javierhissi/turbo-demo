...
describe('user-router', () => {
  ...
  describe(`"POST:${addUsersPath}"`, () => {
    const callApi = (reqBody: TReqBody) => agent.post(addUsersPath).type('form').send(reqBody);
    const userData = {
  ...
  describe(`"PUT:${updateUserPath}"`, () => {
    const callApi = (reqBody: TReqBody) => agent.put(updateUserPath).type('form').send(reqBody);
    const userData = {
      user: User.new('Gordan Freeman', 'gordan.freeman@gmail.com'),
    };
